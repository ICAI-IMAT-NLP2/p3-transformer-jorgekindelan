...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size: int, tgt_vocab_size: int, max_enc_position_embeddings: int, max_dec_position_embeddings: int,
        ...
        self.encoder = TransformerEncoder(vocab_size=src_vocab_size, max_position_embeddings=max_enc_position_embeddings,
                                          d_model=enc_d_model, num_attention_heads=num_attention_heads,
                                          intermediate_size=enc_intermediate_size, num_hidden_layers=num_enc_hidden_layers)
        self.decoder = TransformerDecoder(vocab_size=tgt_vocab_size, max_position_embeddings=max_dec_position_embeddings,
                                          d_model=dec_d_model, num_attention_heads=num_attention_heads,
                                          intermediate_size=dec_intermediate_size, num_hidden_layers=num_dec_hidden_layers)
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token_in_topk = torch.multinomial(probs, num_samples=1)  # indices in [0, k)
            next_token = torch.gather(topk_indices, 1, next_token_in_topk)  # map back to vocab indices
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token = torch.gather(sorted_indices, 1, next_token_in_sorted)
            # Append next token to tgt_input
            tgt_input = torch.cat([tgt_input, next_token], dim=1)
            # Check if all sequences have generated EOS_token
            if (next_token == EOS_token).all():
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input = tgt_input.repeat_interleave(k, dim=0)  # Shape: (k*batch, seq_len)
            next_tokens = topk_indices.view(-1, 1)  # Shape: (k*batch, 1)
            y_candidates = torch.cat([expanded_tgt_input, next_tokens], dim=1)  # Shape: (k*batch, seq_len + 1)
            ...
            h_v = dec_outputs_candidate[:, -1, :]  # Shape: (k*batch, hidden_size)
            h_j = dec_outputs_candidate[:, :-1, :]  # Shape: (k*batch, seq_len, hidden_size)

            # Normalize hidden states
            h_v_norm = F.normalize(h_v, p=2, dim=-1)  # Shape: (k*batch, hidden_size)
            h_j_norm = F.normalize(h_j, p=2, dim=-1)  # Shape: (k*batch, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim = torch.einsum('bd,bld->bl', h_v_norm, h_j_norm)  # Shape: (k*batch, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=-1).values  # Shape: (k*batch,)

            # Compute scores
            P_LM_v = topk_probs.view(-1)  # Shape: (k*batch,)
            scores = (1 - alpha) * torch.log(P_LM_v + 1e-12) - alpha * max_sim  # Shape: (k*batch,)

            # Select the candidate with the highest score
            best_idx = scores.view(batch_size, k).argmax(dim=-1)
            best_token = topk_indices.gather(1, best_idx.view(-1,1))  # Shape: (batch, 1)
            # Append the selected token to the target sequence
            tgt_input = torch.cat([tgt_input, best_token], dim=1)  # Shape: (1, seq_len + 1)

            # Check for EOS_token
            if (best_token == EOS_token).all():
                break
